...
if b==1      % BPSK modulation
   sym_table=exp(1j*[0 -pi]);  sym_table=sym_table([1 0]+1);
   inp=bitseq;   mod_symbols=sym_table(inp+1);   M=2;
 elseif b==2    % QPSK modulation
   sym_table = exp(1j*pi/4*[-3 3 1 -1]); sym_table=sym_table([0 1 3 2]+1);
   inp=reshape(bitseq,b,N_bits/b);
   mod_symbols=sym_table([2 1]*inp+1);   M=4;
 elseif b==3    % generates 8PSK symbols
   sym_table=exp(1j*pi/4*[0:7]); sym_table=sym_table([0 1 3 2 6 7 5 4]+1);
 ...
 elseif b==4    % 16-QAM modulation
   ...
   for k=-3:2:3
      for l=-3:2:3
         m=m+1; sym_table(m) = (k+1j*l)/sq10; % power normalization
      end
   end
