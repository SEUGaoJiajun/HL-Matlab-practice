...
clear, clc; close all; figure(1), clf, figure(2), clf
...
SNRs = [10 20 30];  sq2 = sqrt(2);
MSEs = zeros(length(SNRs),2*3);
for i = 1:length(SNRs)
   SNR = SNRs(i); 
   ...
   for nsym=1:Nsym
      ...
      for k=1:Nfft % 在频域的特定位置加入导频和数据
         if mod(k,Nps)==1
             ...
             X(k) = Data(k-ip); % ip指示了当前OFDM符号中已经加入的导频的数量
         end
      end
      x = ifft(X,Nfft);                            % IFFT
      xt = [x(Nfft-Ng+1:Nfft) x];                  % Add CP|加循环前缀
      h = [(randn+1j*randn) (randn+1j*randn)/2];     % generates a (2-tap) channel|产生一个2抽头信道
