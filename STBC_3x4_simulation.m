...
for i_SNR = 1:length(SNRdBs)
   ...
   for i_packet = 1:N_packets
      ...
      msg_symbol = randi([0 1],L_frame*b,M);
      tx_bits = msg_symbol.'; temp=[]; temp1=[];
      for i=1:4 % time indices
      ...
      for n=1:NT     
         Hr(n,:,:)=(randn(L_frame,NT)+1j*randn(L_frame,NT))/sq2;
      end
      for n=1:NT
         H = reshape(Hr(n,:,:),L_frame,NT);  Hc=conj(H);
         Habs(:,n) = sum(abs(H).^2,2);
         R1n = sum(H.*X1,2)/sq_NT+sigma*(randn(L_frame,1)+1j*randn(L_frame,1));
         R2n = sum(H.*X2,2)/sq_NT+sigma*(randn(L_frame,1)+1j*randn(L_frame,1));
         R3n = sum(H.*X3,2)/sq_NT+sigma*(randn(L_frame,1)+1j*randn(L_frame,1));
         R4n = sum(H.*X4,2)/sq_NT+sigma*(randn(L_frame,1)+1j*randn(L_frame,1));
         R5n = sum(H.*X5,2)/sq_NT+sigma*(randn(L_frame,1)+1j*randn(L_frame,1));
         R6n = sum(H.*X6,2)/sq_NT+sigma*(randn(L_frame,1)+1j*randn(L_frame,1));
         R7n = sum(H.*X7,2)/sq_NT+sigma*(randn(L_frame,1)+1j*randn(L_frame,1));
         R8n = sum(H.*X8,2)/sq_NT+sigma*(randn(L_frame,1)+1j*randn(L_frame,1));
