...
for i_SNR = 1:length(SNRdBs)
   ...
   for i_packet = 1:N_Packets
      ...
      R1 = sum(H.*X1,2)/sq_NT + sigma*(randn(L_frame,1)+1j*randn(L_frame,1));
      R2 = sum(H.*X2,2)/sq_NT + sigma*(randn(L_frame,1)+1j*randn(L_frame,1));
...
hold on; grid on;  xlabel('SNR[dB]'); ylabel('BER');
