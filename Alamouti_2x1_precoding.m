...
for i_SNR=1:length(SNRdBs)
   ...
   for i_packet=1:N_packet
      ...
      msg_bit = randi([0 M-1],N_pbits/b,1);
      %%%%%%%%%%%%% Transmitter %%%%%%%%%%%%%%%%%%
      % s = modulate(mod_obj,msg_bit);
      s = qammod(msg_bit,M,'gray');
      ...
      %%%%%%%%%%%%% Channel and Noise %%%%%%%%%%%%%
      H = (randn(NR,T_TX)+1j*randn(NR,T_TX))/sq2;
      for i=1:code_length
      ...
      for i=1:N_frame
         Rx(:,:,i) = He*Tx_symbol(:,:,i)+sigma*(randn(NR,2)+1j*randn(NR,2));            
      end             
      %%%%%%%%%%%%% Receiver %%%%%%%%%%%%%%%%%%
      for i=1:N_frame
