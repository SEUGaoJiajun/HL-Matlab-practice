   ...
   for ncf = 0:2 % no/clip/clip&filter
      ...
      for cr = 1:N_CR
         ...
         for nblk = 1:Nblk %(i)
            ...
            msgbin = randi(M,1,N)-1;   % binary squences
            % X = A*modulate(mdmod,msgbin); % 4QAM (QPSK) mapper
            X = qammod(msgbin,M,'gray')*A;
            X(1) = 0+1j*0;    % DC subcarrier not used
            x = IFFT_oversampling(X,N,L);
            x_b = add_CP(x,Ncp*L);
            x_b_os = [zeros(1,(N/2-Ncp)*L), x_b, zeros(1,N*L/2)];
            x_p = sqrt(2)*real(x_b_os.*exp(1j*2*wc*t));
            ...
            y_b = sqrt(2)*y_p_n.*exp(-1j*2*wc*t); 
            Y_b = fft(y_b);
            ...
            % Y_b_bin = qamdemod(Y_b_f/A, M, 'gray');
